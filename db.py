...
#     cur = conn.cursor()
#     rows = [
#         (int(m.get("id")), group_link, str(m.get("date")),
#          None if m.get("sender_id") is None else int(m.get("sender_id")),
#          (m.get("text") or ""))
#         for m in messages
#     ]
...
#     conn = _get_connection()
#     cur = conn.cursor()
#     cur.execute("SELECT COUNT(*) FROM groups WHERE substr(joined_at,1,10)=?;", (date_str,))
#     return int(cur.fetchone()[0])

# def _today_joins_for_phone_sync(date_str: str, phone: str) -> int:
#     conn = _get_connection()
#     cur = conn.cursor()
#     cur.execute("SELECT COUNT(*) FROM groups WHERE substr(joined_at,1,10)=? AND account_phone=?;", (date_str, phone))
#     return int(cur.fetchone()[0])
...
# Async helper for threads (backwards compatible)
try:
    _to_thread = asyncio.to_thread
except AttributeError:
    async def _to_thread(func, *args, **kwargs):
...
def _get_connection() -> sqlite3.Connection:
    ...
    if _connection is None:
        ...
        # remove leftover WAL/SHM files if DB file exists
        for ext in ["-wal", "-shm"]:
            path = DB_PATH + ext
...
def _record_group_join_sync(link: str, joined_at: str, account_phone: str) -> None:
    try:
        ...
        print(f"[DB ERROR] record_group_join_sync: {e}")
...
def _record_messages_sync(group_link: str, messages: List[Dict]) -> None:
    ...
    try:
        ...
        print(f"[DB ERROR] record_messages_sync: {e}")
